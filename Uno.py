...
class Deck:
    ...
    def __init__(self):
        ...
        self.discard = []
        for i in range(4):
            self.cards.append( Card(color[i], "0"))#添加4張0
        #數字1~9和Skip", "Reverse", "+2"都各有兩張
        for c in color:
            for r in rank:
                for _ in range(2): self.cards.append(Card(c, r))
...
def correctNowPlayer(nowPlayer, playerNumber): # 修正nowPlayer
    if nowPlayer > playerNumber-1 :
        nowPlayer-=playerNumber
        return nowPlayer
    elif nowPlayer < 0:
        nowPlayer+=playerNumber
        return nowPlayer
    else: return nowPlayer
...
class Player:
    ...
    def humanPlay(self, deck):
        ...
        # 循環直到用戶輸入合法的整數在指定範圍內
        while (True):
            p(self.name+"請問你要出第幾張牌?(如果不想出牌，請輸入0)")
            s = input()
            # 檢查用戶輸入是否為整數
            if s.isdigit():
                userInput = int(s)
                ...
                if userInput >= 0 and userInput <= len(self.hand):
                    ...
                    if card.getColor()==dPC.getColor() or card.getRank()==dPC.getRank() or card.getColor()=="special":
                        ...
                        p("請輸入顏色為"+dPC.getColor()+"或special的牌或者點數為"+dPC.getRank()+"的牌!")
                else:
                    p("請輸入0~"+len(self.hand)+"之間的整數!")   
            else:
                p("請輸入一個有效的整數！")
    ...
    def sayCardNum(self): p(self.name+"說:UNO!") if self.cardNum()==1 else p(self.name+"剩"+str(self.cardNum())+"張")
...
def UNO(playerNumber, human_name):
    ...
    while True:
        ...
        if condition == "normal":
            ...
            nowPlayer += direction
            nowPlayer = (nowPlayer + playerNumber) % playerNumber # 自動迴圈控制玩家索引
            p(players[nowPlayer].name+"抽2張牌")
            players[nowPlayer].deal(2,deck)
        elif condition == "+4":
            nowPlayer += direction
            nowPlayer = (nowPlayer + playerNumber) % playerNumber # 自動迴圈控制玩家索引
            p(players[nowPlayer].name+"抽4張牌")
        ...
        p("下一個人是"+players[nowPlayer].name+"\n====================")
